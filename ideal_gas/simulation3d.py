...
class Particle:
    ...
    def compute_refl(self, step, size):
        ...
        if abs(x[1])-r < projy or abs(size-x[1])-r < projy:
            self.velocity[1] *= -1.
        if abs(x[2])-r < projz or abs(size-x[2])-r < projz:
            self.velocity[2] *= -1.
...
def init_list_random(N, radius, mass, boxsize):
    ...
    for i in range(N):
        ...
        vx = float(np.random.rand(1)*6)
        vy = float(np.random.rand(1)*6)
        vz = float(np.random.rand(1)*6)
        v = [vx, vy, vz]

        x = float(radius + np.random.rand(1)*(boxsize-2*radius))
        y = float(radius + np.random.rand(1)*(boxsize-2*radius))
        z = float(radius + np.random.rand(1)*(boxsize-2*radius))
        pos = [x, y, z]
...
particle_list = init_list_random(particle_number, radius = 2, mass = 1, boxsize = boxsize)
...
#pd.Dataframe(self.solpos).to_csv("data_pos")

all_v = np.zeros((stepnumber + 1, 3 * len(particle_list)))
all_pos = np.zeros((stepnumber + 1, 3 * len(particle_list)))

for i in range(len(particle_list)):
    all_v[:, 3*i: 3*i + 3] = particle_list[i].solvel
    all_pos[:, 3*i: 3*i + 3] = particle_list[i].solpos
